...
`DeviceIdProvider`, `AuthTokenProvider` et `PocketBaseService` s'injectent directement via `@Inject constructor`.
...
Lu par `app/build.gradle.kts` → `BuildConfig.POCKETBASE_URL`, par buildType :
- **debug** : URL depuis `local.properties` (auto-détectée par `dev-local.sh`)
- **release** : `https://pocketbase.bauchbauch.ovh` (hardcodé)
...
│   │       └── TerraceMapper.kt                  ✅
│   ├── preferences/
│   │   ├── DeviceIdProvider.kt                   ✅ (UUID persistant)
│   │   └── AuthTokenProvider.kt                  ✅ (token JWT + mot de passe device)
│   ├── remote/
...
└── 1742880000_devices_auth.js                    ✅ (collection devices + rules auth requise)
```
...
### Sprint 8 — Authentification anonyme PocketBase ✅
**Objectif** : Seuls les appareils enregistrés peuvent accéder aux données (pas de secret dans l'APK).

**Contenu** :
- `AuthTokenProvider` : stocke le token JWT + mot de passe dans SharedPreferences (`auth_prefs`)
- `PocketBaseService.ensureAuth()` : enregistrement au 1er lancement (`{deviceId}@terrasse.app` + mot de passe UUID aléatoire), fall back sur login si déjà enregistré
- `checkResponse()` : vérifie le code HTTP sur toutes les réponses, efface le token sur 401, lit le body d'erreur PocketBase via `errorStream`
- `ensureAuth()` appelé dans `syncFromServer()` ET dans la boucle de retry de `startRealtimeSync()` → re-auth automatique après expiration ou boot offline
- Migration `1742880000_devices_auth.js` : collection `devices` (auth, registration ouverte) + rules `@request.auth.id != ''` sur terraces et votes
- URL de prod configurée dans `release` buildType : `https://pocketbase.bauchbauch.ovh`

**Comportement offline** :
- 1er lancement sans réseau → OFFLINE (impossible de s'enregistrer)
- Lancement suivant avec token → fonctionne offline (token déjà stocké)
- Token expiré (7j) → effacé sur 401 → re-login au prochain cycle SSE

**Validation sur device** :
- [x] Un nouvel appareil apparaît dans la collection `devices` de PocketBase au 1er lancement
- [x] `fetchAllTerraces` retourne les vraies données (pas une liste vide)
- [x] Un appel sans token retourne 200 + liste vide (comportement PocketBase) → `checkResponse` lève une exception pour forcer le retry
...
### Situation actuelle
- Collection `devices` (auth) avec enregistrement anonyme ouvert
- Token JWT stocké dans SharedPreferences (`auth_prefs`), non-extractible depuis l'extérieur de l'app
- Toutes les API rules requièrent `@request.auth.id != ''`
- Aucun secret statique dans l'APK

### Amélioration recommandée (post-MVP)
